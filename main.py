...
def check_for_changes(cwd:str, package:str) -> list | None:
    ...
    status_logger("Checking for Changes", f"Checking for changes in the {package} repository.")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not sha:
                ...
                status_logger(custom_subject, custom_message, logging_level =  logging.WARNING)
            ...
            if stored_sha != latest_commit_sha:
                ...
                status_logger("No Changes Detected", f"No changes detected for {package} since the last check.")
...
def parent_directory_validation() -> str | None:
    ...
    status_logger("Parent Directory Validation", "Validating the parent directory specified in the .env file.")
    ...
    if not is_valid_directory(parent_directory):
        ...
        status_logger(custom_subject, f"The parent directory {parent_directory} is invalid.", logging_level = logging.ERROR)
...
def is_git_repo(cwd:str) -> bool:
    ...
    status_logger("Git Repository Validation", f"Checking if the directory {cwd} is a valid git repository.")
    ...
    if is_existing_directory(git_path):
        ...
        if current_branch == "HEAD":
            ...
            status_logger("Git Repository Error", f"The repository at {cwd} is in a detached HEAD state.", logging_level = logging.WARNING)
...
def run_me_them_commands(cwd:str, package_name:str) -> bool:
    ...
    try:
        ...
        if not git_status_porcelain.stdout.strip():
            ...
            status_logger("No Changes to Commit", f"No changes detected in the {package_name} repository since the last commit.")
        ...
        status_logger("Changes Committed", f"Changes committed to the {package_name} repository.")
