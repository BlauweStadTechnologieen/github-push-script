...
def company_signoff():
    return f"""
    Yours sincerely<br>
    <b>{email_auth.sender_name}</b><br>
    {email_auth.sender_department} Team<br>
    {email_auth.tech_department}<br><br>
    """
...
def send_message(latest_commit_data:dict, changed_repo_list:list, github_owner:str) -> None:
    ...
    message_body = f"""
    Dear {email_auth.receiver_name}<br><br>
    ...
    * You must be logged into the GitHub Repository in order to see the list of commits within the API call.<br><br>
    {company_signoff()}
    """
    email_auth.smtp_auth(message_body,"Github Commit Report")
